...
#define BBIOSAPI_SIGNATURE_PHYS_START_ADDR                                     \
	0xFFE00000 // Defining the Physical start address for the search
#define BBIOSAPI_SIGNATURE_SEARCH_AREA                                         \
	0x001FFFFF // Defining the Memory search area size
#define BBAPI_BUFFER_SIZE                                                      \
	256 // maximum size of a buffer shared between user and kernel space
...
extern unsigned int bbapi_rw(uint32_t group, uint32_t offset, void __kernel *in,
			     uint32_t size_in, void __kernel *out,
			     uint32_t size_out, uint32_t *bytes_written);
